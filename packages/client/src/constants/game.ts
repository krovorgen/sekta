export const GAME_OPTIONS = {
  // настройки отображения
  GAME_DEBUG: false,
  GAME_MIN_FPS: 25,
  GAME_MAIN_FPS: 60,
  CANVAS_WIDTH: 1280,
  CANVAS_HEIGHT: 720,
  CANVAS_OFFSET: 0.15,
  FLOOR_HEIGHT: 75,
  // настройки физики игры
  GRAVITY_VALUE: 30,
  FRICTION_FORCE: 0.05,
  SPEED_INIT: 1.0,
  SPEED_STEP: 0.025,
  // настройки персонажа
  PLAYER_WIDTH: 50,
  PLAYER_HEIGHT: 100,
  PLAYER_SPEED: 6.5,
  PLAYER_LEFT_OFFSET: 100,
  PLAYER_JUMP_MULTIPLIER: 2.2,
  PLAYER_JUMP_DURATION: 200,
  PLAYER_JUMP_BLOCKTIME: 1500,
  PLAYER_SLIDE_MULTIPLIER: 2.0,
  PLAYER_SLIDE_DURATION: 600,
  PLAYER_SLIDE_BLOCKTIME: 1500,
  // настройки ловушек
  TRAP_WIDTH: 20,
  TRAP_HEIGHT: 20,
  TRAP_SPEED: 165,
  TRAP_TIME: 3000,
  // настройки бонусов
  BONUS_WIDTH: 20,
  BONUS_HEIGHT: 20,
  BONUS_SPEED: 165,
  BONUS_TIME: 3000,
  FIRE_IMMUNITY_DURATION: 10000,
  SPEED_SLOWDOWN_DURATION: 3000,
  // настройки огненного дождя
  FIREBALL_WIDTH: 20,
  FIREBALL_HEIGHT: 20,
  FIREBALL_SPEED_MIN: 300,
  FIREBALL_SPEED_MAX: 400,
  // настройки фона игры
  BACKGROUND_LANDSCAPE_SPEED: 75,
  BACKGROUND_ROAD_SPEED: 165,
  BACKGROUND_SKY_SPEED: 50,
  BACKGROUND_TIME: 20000,
  // настройки звукового сопровождения
  MASTER_SOUND_VOLUME: 1.0,
  BACKGROUND_SOUND_VOLUME: 0.5,
}
